...
import { Image as ImageIcon, Minus, Plus, ArrowLeft, Star } from "lucide-react";
        ...
        <div className="mt-5 grid lg:grid-cols-[1.1fr_1fr] gap-6 lg:gap-10">
          <div className="space-y-3">
            <div className="relative aspect-square rounded-[28px] bg-tile-mint flex items-center justify-center overflow-hidden">
              ...
              <div className="absolute top-5 left-5 inline-flex items-center gap-2 rounded-full bg-background/90 backdrop-blur px-3 py-1.5 text-xs font-medium">{product.brand}</div>
              <div className="absolute bottom-5 left-5 right-5 flex items-end justify-between gap-3">
                <div className="rounded-full bg-ink text-ink-foreground px-4 py-2 text-sm font-semibold">{formatNGN(product.price)}</div>
                <div className="text-right text-xs text-foreground/70">See more</div>
              </div>
            </div>
            <div className="grid grid-cols-4 gap-3">
              {["bg-tile-butter", "bg-tile-peach", "bg-tile-sky", "bg-tile-mist"].map((c, i) => (
          ...
          <div className="lg:pt-3">
                ...
                <div key={v.type}>
                  ...
                  <div className="flex flex-wrap gap-2 mt-2.5">
                    {v.values.map((val) => {
                      ...
                      return (<button key={val} onClick={() => setSelected({ ...selected, [v.type]: val })} className={cn("h-10 min-w-12 px-4 rounded-full border text-sm font-medium transition-all", isSel ? "bg-ink text-ink-foreground border-ink" : "bg-background text-foreground border-border hover:border-foreground/30")}>{val}</button>);
                    })}
                  </div>
                </div>
              ))}
            ...
            <div className="flex items-center gap-3 mt-7">
              <div className="inline-flex items-center bg-muted rounded-full p-1">
                <button onClick={() => setQty(Math.max(1, qty - 1))} className="h-9 w-9 rounded-full bg-background flex items-center justify-center hover:bg-background/80 transition-colors"><Minus className="h-3.5 w-3.5" /></button>
                <span className="px-4 text-sm font-medium w-10 text-center">{qty}</span>
                <button onClick={() => setQty(qty + 1)} className="h-9 w-9 rounded-full bg-background flex items-center justify-center hover:bg-background/80 transition-colors"><Plus className="h-3.5 w-3.5" /></button>
              </div>
            ...
            <Accordion type="single" collapsible className="mt-6" defaultValue="desc">
              ...
              <AccordionItem value="ship" className="border-b-0 border-t border-border/60">
                ...
                <AccordionContent className="text-sm text-muted-foreground leading-relaxed">Free pickup available. Delivery within 2-5 business days.</AccordionContent>
              </AccordionItem>
            </Accordion>
          </div>
        </div>
