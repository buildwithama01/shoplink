      ...
      <div className="divide-y divide-border/60">
        {[{ label: "Bank Name", value: bankDetails.bankName }, { label: "Account Name", value: bankDetails.accountName }, { label: "Account Number", value: bankDetails.accountNumber }].map((row) => (
          <div key={row.label} className="flex items-center justify-between px-5 py-3.5 gap-3">
            <div><div className="text-xs text-muted-foreground">{row.label}</div><div className="font-semibold text-sm mt-0.5 font-mono tracking-wide">{row.value}</div></div>
    ...
    <div className="min-h-screen bg-canvas">
      ...
      <div className="container py-8">
        ...
        <div className="grid lg:grid-cols-[1.6fr_1fr] gap-6 mt-7">
          <div className="space-y-5">
            ...
            <Section step={2} title="Shipping method">
              <RadioGroup value={shipping} onValueChange={(v: "pickup" | "delivery") => setShipping(v)} className="grid sm:grid-cols-2 gap-3">
                {[{ v: "pickup", label: "Pickup", note: "Free", desc: "Collect from our Lagos store" }, { v: "delivery", label: "Delivery", note: "₦5,000", desc: "2-5 business days" }].map((o) => (
                  <Label key={o.v} htmlFor={`ship-${o.v}`} className={cn("flex items-start gap-3 border rounded-2xl p-4 cursor-pointer transition-all", shipping === o.v ? "border-ink bg-muted/40" : "border-border hover:border-foreground/30")}>
                    <RadioGroupItem id={`ship-${o.v}`} value={o.v} className="mt-0.5" />
                    <div className="flex-1"><div className="flex items-center justify-between gap-2"><span className="font-medium text-sm">{o.label}</span><span className="text-xs font-semibold">{o.note}</span></div><div className="text-xs text-muted-foreground mt-0.5">{o.desc}</div></div>
                  </Label>
                ))}
              </RadioGroup>
            </Section>
              ...
              <Section step={3} title="Delivery address">
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <div><Label className="text-xs text-muted-foreground">State</Label>
                    <Select><SelectTrigger className="mt-1.5 rounded-xl"><SelectValue placeholder="Select state" /></SelectTrigger><SelectContent><SelectItem value="lagos">Lagos</SelectItem><SelectItem value="abuja">Abuja</SelectItem><SelectItem value="rivers">Rivers</SelectItem><SelectItem value="kano">Kano</SelectItem></SelectContent></Select>
                  </div>
                </div>
              </Section>
            )}
          ...
          <div className="lg:col-span-1"><OrderSummaryCard shippingFee={fee} /></div>
        </div>
      </div>
    </div>
  );
