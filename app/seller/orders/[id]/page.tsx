...
export default async function SellerOrderDetailPage({
  ...
  timelineSteps.push({
    label: "Order placed",
    timestamp: new Date(order.created_at).toLocaleString("en-US", {
      month: "short",
      day: "numeric",
      hour: "numeric",
      minute: "2-digit",
  ...
  if (order.status !== "pending" && order.status !== "cancelled") {
    timelineSteps.push({
      ...
      timestamp: new Date(order.updated_at || order.created_at).toLocaleString(
        "en-US",
  ...
  if (
    ...
    timelineSteps.push({
      label: "Order shipped",
      timestamp: new Date(order.updated_at).toLocaleString("en-US", {
        month: "short",
        day: "numeric",
        hour: "numeric",
        minute: "2-digit",
            ...
            <Panel title="Customer information">
              <dl className="grid sm:grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <dd className="font-medium mt-0.5">{order.customer_name}</dd>
                </div>
                <div>
                  <dt className="text-xs text-muted-foreground">Email</dt>
                  <dd className="font-medium mt-0.5">{order.customer_email}</dd>
                </div>
                <div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground mt-0.5">
                        {it.variant_label || "Default"} · Qty {it.quantity}
                      </div>
                    </div>
                    <div className="font-medium text-sm">
                      {formatNGN(it.total_price)}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <Panel title="Order summary">
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span>{formatNGN(itemsTotal)}</span>
                </div>
                <div className="flex justify-between">
