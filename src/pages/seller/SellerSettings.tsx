...
const PLANS = [
  ...
  },
  {
    ...
    price: "₦2,500/mo",
    orders: 250,
    ordersLabel: "250 orders/mo",
    color: "bg-tile-butter",
    textColor: "text-foreground",
    icon: TrendingUp,
      ...
      <div className="bg-background rounded-[24px] border border-border/60 shadow-2xl w-full max-w-md p-6 animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-start justify-between mb-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="font-semibold text-base">Delete your account</h2>
              <p className="text-sm text-muted-foreground mt-1">
                This will permanently delete your store, all products, and order history. This action <strong>cannot</strong> be undone.
              </p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="h-8 w-8 rounded-full bg-muted flex items-center justify-center hover:bg-muted/70 transition-colors shrink-0 ml-2"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
...
export default function SellerSettings() {
  ...
  const currentPlanId = "hustle"; // mock: seller is on Hustle
  const currentPlan = PLANS.find((p) => p.id === currentPlanId)!;
  const ordersUsed = stats.totalOrders; // 47 from mock
    ...
    <>
      <SellerLayout>
        <SellerTopBar
          ...
          action={<Button onClick={() => toast.success("Changes saved!")}>Save changes</Button>}
        />

        <div className="p-7">
          <Tabs defaultValue="general">
            <TabsList className="bg-muted rounded-full p-1 h-auto">
              {["general", "shipping", "payments", "plan", "seo"].map((t) => (
                <TabsTrigger
                  key={t}
                  value={t}
              ...
              <Panel title="Store information">
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input defaultValue={store.name} className="mt-1.5 rounded-xl" />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <Input defaultValue={store.whatsapp} className="mt-1.5 rounded-xl" />
                  </div>
                  <div>
                ...
                <div className="mt-4 flex items-start justify-between gap-4">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground mt-0.5">
                      Permanently remove your store, products, and all data.
                    </div>
                  </div>
                  <Button
                    variant="destructive"
              ...
              <div className="grid md:grid-cols-2 gap-5">
                <Panel title="Pickup">
                  <div className="space-y-3">
                    <div>
                      ...
                      <Input defaultValue="Pickup" className="mt-1.5 rounded-xl" />
                    </div>
                ...
                <Panel title="Delivery">
                  <div className="space-y-3">
                    <div>
                      <Label className="text-xs text-muted-foreground">Label</Label>
                      <Input defaultValue="Delivery" className="mt-1.5 rounded-xl" />
                    </div>
                    <div>
                      <Label className="text-xs text-muted-foreground">Fee</Label>
                      <Input defaultValue="₦5,000" className="mt-1.5 rounded-xl" />
                    </div>
                    <div className="flex items-center justify-between pt-2">
                      <span className="text-sm font-medium">Active</span>
                      <Switch defaultChecked />
                  ...
                  <div className="pt-4 border-t border-border/60 mt-4 space-y-4">
                    ...
                    <div className="grid sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <Input defaultValue="Access Bank" className="mt-1.5 rounded-xl" />
                      </div>
                      <div>
                        <Label className="text-xs text-muted-foreground">Account Name</Label>
                        <Input defaultValue="Neon Supply Ltd" className="mt-1.5 rounded-xl" />
                      </div>
                      <div className="sm:col-span-2">
                        <Label className="text-xs text-muted-foreground">Account Number</Label>
                        <Input defaultValue="0123456789" className="mt-1.5 rounded-xl" />
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <TabsContent value="plan" className="mt-6 space-y-5">
              ...
              <div className={cn("rounded-[20px] p-6 border border-border/60", currentPlan.color)}>
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="text-xs uppercase tracking-wider text-foreground/60 font-medium mb-1">
                      Current plan
                    </div>
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <div className="text-2xl font-bold mt-0.5">
                      {ordersUsed}
                      {ordersLimit && (
                        <span className="text-sm font-normal text-muted-foreground">/{ordersLimit}</span>
                      )}
                    </div>
                  </div>
                </div>
                    ...
                    <div className="h-2 rounded-full bg-background/60 overflow-hidden">
                      <div
                        className={cn(
                          ...
                          usagePercent >= 90 ? "bg-destructive" : usagePercent >= 70 ? "bg-amber-500" : "bg-foreground/70"
                        )}
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          isCurrent ? "border-foreground/30 bg-muted/40" : "border-border hover:border-foreground/20"
                        )}
                      >
                        {isCurrent && (
                          <span className="absolute top-3 right-3 inline-flex items-center gap-1 bg-foreground text-background text-[10px] font-semibold px-2 py-0.5 rounded-full">
                        ...
                        {!isCurrent && (
                          <Button
                            ...
                            onClick={() =>
                              toast.info(`Upgrade to ${plan.name} — contact support or pay online.`)
                            }
                          >
              ...
              <Panel title="SEO">
                <div className="space-y-4">
                  <div>
                    ...
                    <Input
                      defaultValue="Cruz Gadgets — Future Forward Gadgets"
                  ...
                  <div>
                    ...
                    <Input
                      defaultValue="Shop the latest phones, laptops & accessories."
                      className="mt-1.5 rounded-xl"
                    />
                  </div>
                </div>
