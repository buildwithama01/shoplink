...
function PlanBadge({ planId }: { planId: PlanId }) {
  ...
  return (
    <span className={cn("inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-semibold", plan.badge)}>
...
function ChangePlanDialog({
  ...
  store: typeof initialStores[number];
  onSave: (planId: PlanId) => void;
  onClose: () => void;
}) {
        ...
        <div className="space-y-2">
          {PLANS.map((plan) => {
            ...
            return (
              ...
              >
                <div className={cn("h-9 w-9 rounded-xl flex items-center justify-center shrink-0", plan.color)}>
                  <Icon className="h-4 w-4" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-sm">{plan.name}</div>
                  <div className="text-xs text-muted-foreground">{plan.orders} · {plan.price}</div>
                </div>
                <div
                  className={cn(
      ...
      <div className="bg-background rounded-[24px] border border-border/60 shadow-2xl w-full max-w-md p-6 animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-start gap-3 mb-5">
          ...
          <div className="flex-1">
            <h2 className="font-semibold text-base">Delete store</h2>
            <p className="text-sm text-muted-foreground mt-1">
              This will permanently delete <strong>{store.name}</strong> and all associated data. This cannot be undone.
            </p>
          </div>
          <button
            onClick={onClose}
            className="h-8 w-8 rounded-full bg-muted flex items-center justify-center hover:bg-muted/70 transition-colors shrink-0"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="space-y-2 mb-5">
          <label className="text-xs text-muted-foreground">
            Type <span className="font-mono font-bold text-foreground">{store.name}</span> to confirm
          </label>
          <input
            autoFocus
            value={confirmText}
            ...
            className="w-full rounded-xl border border-border bg-muted/40 px-4 py-2.5 text-sm outline-none focus:border-destructive/50 focus:ring-2 focus:ring-destructive/20 transition-all"
          />
        </div>
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full mt-1 z-20 w-44 rounded-2xl border border-border bg-background shadow-lg p-1.5 animate-in fade-in slide-in-from-top-2 duration-150">
            <button
              className="w-full text-left px-3 py-2 rounded-xl text-sm hover:bg-muted transition-colors"
            ...
            <button
              className="w-full text-left px-3 py-2 rounded-xl text-sm text-destructive hover:bg-destructive/10 transition-colors"
...
export default function AdminStores() {
  ...
  const toggleStatus = (id: number) => {
    ...
    toast.success(`${store.name} ${store.status === "Active" ? "deactivated" : "activated"}`);
  };
  ...
  const changePlan = (id: number, planId: PlanId) => {
    ...
    toast.success(`${storeName} moved to ${planName}`);
    setPlanDialog(null);
  };
  ...
  const deleteStore = (id: number) => {
    ...
    toast.error(`${storeName} has been deleted`);
  ...
  return (
    <>
      ...
      {planDialog && (
        <ChangePlanDialog
          store={planDialog}
          onSave={(planId) => changePlan(planDialog.id, planId)}
      ...
      {deleteDialog && (
        <DeleteStoreDialog
          store={deleteDialog}
          onConfirm={() => deleteStore(deleteDialog.id)}
