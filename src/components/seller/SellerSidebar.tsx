...
const navItems = [
  ...
  { to: "/seller/categories", icon: Tag, label: "Categories" },
  ...
  return (
    <aside className="hidden md:flex flex-col w-64 shrink-0 p-5 pr-0">
      <div className="flex flex-col h-full bg-background rounded-[24px] border border-border/60 p-5">
        <Link to="/seller" className="flex items-center gap-2.5 px-1">
        ...
        <div className="mt-6">
          ...
          <Link to="/seller/products/new" className="flex items-center gap-2 px-3 h-9 rounded-full text-sm text-foreground hover:bg-muted transition-colors">
            <Plus className="h-4 w-4" /> Add product
          </Link>
          <Link to="/seller/categories" className="flex items-center gap-2 px-3 h-9 rounded-full text-sm text-foreground hover:bg-muted transition-colors">
        ...
        <div className="mt-6">
          ...
          <div className="space-y-1">
            {recent.map((o) => (
              <Link key={o.id} to={`/seller/orders/${o.id}`} className="flex items-center gap-2.5 px-3 py-2 rounded-2xl hover:bg-muted transition-colors">
        ...
        <div className="mt-auto pt-4 border-t border-border/60 space-y-1">
          ...
          <Link to="/seller/login" className="flex items-center gap-2 px-3 h-9 rounded-full text-sm text-muted-foreground hover:text-foreground hover:bg-muted transition-colors">
...
export function SellerLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen flex w-full bg-canvas pb-[72px] md:pb-0">
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-background/90 backdrop-blur-lg border-t border-border/60 px-2 py-2 flex items-center justify-around z-50">
        {navItems.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
