...
export default function AdminUsersPage() {
  ...
  const toggleUser = (id: number) => { setUsers(users.map(u => u.id === id ? { ...u, active: !u.active } : u)); };
            ...
            <TableBody>
              {users.map((u) => (
                <TableRow key={u.id}>
                  ...
                  <TableCell className="text-right"><Button variant="outline" size="sm" onClick={() => toggleUser(u.id)}>{u.active ? "Suspend" : "Restore"}</Button></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
